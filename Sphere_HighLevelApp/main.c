...
static void getResponse(int fd)
{
    ...
    while (1)
    {
        ...
        if (receiveBuffer[bytesRead - 2] == 0x0D) //detect return
        {

            rxData[rxIndex - 1] = 0;
            Log_Debug("Full UART received: '%s'.\n", (char*)rxData);
            token = strtok((char*)rxData, "\r");

            while(token!=NULL)// ignore others, only take the last response
            {
                strcpy(Data, token);
                token = strtok(NULL, "\r");
            }

            Log_Debug("Real data received: '%s'.\n", (char*)Data);
            rxIndex = 0;
            return;
        }
    }
...
static void OBD2(void)
{
    SendUartMessage(uartFd, engineRPMCmd);
    Log_Debug("Waiting Engine RPM\n");
    getResponse(uartFd);
    engineRPM = ((strtol(&Data[6], 0, 16) * 256) + strtol(&Data[9], 0, 16)) / 4;
    ...
    getResponse(uartFd);
    engineFuel = strtol(&Data[6], 0, 16); // in the scale of 255
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    static const struct timespec sendPeriodMQTT = { .tv_sec = 0, .tv_nsec = MQTT_PUBLISH_PERIOD };
